---
title: "Using {ggchicklet2}"
output: 
  rmarkdown::html_vignette:
    df_print: kable
vignette: >
  %\VignetteIndexEntry{Using {ggchicklet2}}
...
has_forcats    <- requireNamespace("forcats",    quietly = TRUE)
```
...
The rounded corners aesthetic looked great and said feature begat the creation of {ggchicklet}. {ggchicklet2} is a friendly fork that carries that aesthetic forward to additional geoms (see also `geom_chicklet_boxplot()`).

Let's load up the packages we'll need:

```{r setup, eval = has_hrbrthemes && has_dplyr && has_forcats}
library(hrbrthemes)  # preferred theme
library(ggchicklet2) # this package!
library(dplyr)       # data wrangling
library(forcats)     # factor wrangling
library(ggplot2)
```

```{r setup-fallback, eval = !(has_hrbrthemes && has_dplyr && has_forcats), echo = FALSE, results = "asis"}
cat("> *Suggested packages (`hrbrthemes`, `dplyr`, `forcats`) are not installed; the remaining code chunks in this vignette are skipped. Install them to see the rendered example.*\n")
```
...
```{r data, eval = has_hrbrthemes && has_dplyr && has_forcats}
data("debates2019")
...
```{r data-ex-01, eval = has_hrbrthemes && has_dplyr && has_forcats}
debates2019 %>% 
...
```{r data-ex-02, eval = has_hrbrthemes && has_dplyr && has_forcats}
debates2019 %>% 
  filter(debate_group == 1) %>% 
  filter(night == 1) %>% 
  distinct(topic) %>% 
...
```{r chicklet, fig.width=600/72, fig.height=600/72, eval = has_hrbrthemes && has_dplyr && has_forcats}
debates2019 %>%
  filter(debate_group == 1) %>% 
